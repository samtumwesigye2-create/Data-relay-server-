...
from durable_delivery import DeliveryPlanner
from delivery_runtime import SQLiteDeliveryStore
APP_NAME='UNG-PULSAR Data Relay';DB=os.environ.get('DRS_DB','data_relay.db');API_KEY=os.environ.get('DRS_API_KEY','');RETENTION_DAYS=int(os.environ.get('DRS_RETENTION_DAYS','90'));CPU_ALERT=float(os.environ.get('DRS_CPU_ALERT','90'));MEMORY_ALERT=float(os.environ.get('DRS_MEMORY_ALERT','90'));DISK_ALERT=float(os.environ.get('DRS_DISK_ALERT','90'));SLOW_QUERY_MS=float(os.environ.get('DRS_SLOW_QUERY_MS','1000'));REPLICATION_LAG_SECONDS=float(os.environ.get('DRS_REPLICATION_LAG_SECONDS','30'));BRUTE_FORCE_ATTEMPTS=int(os.environ.get('DRS_BRUTE_FORCE_ATTEMPTS','5'));BRUTE_FORCE_WINDOW_SECONDS=int(os.environ.get('DRS_BRUTE_FORCE_WINDOW_SECONDS','300'));CATEGORIES={'system_metric','application_log','user_interaction','database_activity','api_call','security_event','error','communication','file_transfer','alert_prediction'};SECRET_KEYS={'password','passwd','secret','token','access_token','refresh_token','authorization','cookie','set-cookie','api_key','apikey','private_key'}
...
def scrub(v):
 ...
 return v
def init():
 c=conn();c.executescript('''CREATE TABLE IF NOT EXISTS events(id TEXT PRIMARY KEY,category TEXT NOT NULL,source TEXT NOT NULL,severity TEXT NOT NULL,actor TEXT,action TEXT,resource TEXT,status TEXT,duration_ms REAL,trace_id TEXT,payload_json TEXT NOT NULL,created_at REAL NOT NULL,prev_hash TEXT,event_hash TEXT NOT NULL);CREATE INDEX IF NOT EXISTS idx_events_time ON events(created_at DESC);CREATE INDEX IF NOT EXISTS idx_events_category ON events(category,created_at DESC);CREATE INDEX IF NOT EXISTS idx_events_actor ON events(actor,created_at DESC);CREATE INDEX IF NOT EXISTS idx_events_trace ON events(trace_id);CREATE TABLE IF NOT EXISTS audit(id TEXT PRIMARY KEY,who TEXT NOT NULL,what TEXT NOT NULL,resource TEXT,result TEXT,how_method TEXT,how_channel TEXT,ip_address TEXT,user_agent TEXT,device_id TEXT,session_id TEXT,trace_id TEXT,details_json TEXT NOT NULL,occurred_at REAL NOT NULL,recorded_at REAL NOT NULL,prev_hash TEXT,event_hash TEXT NOT NULL);CREATE INDEX IF NOT EXISTS idx_audit_when ON audit(occurred_at DESC);CREATE INDEX IF NOT EXISTS idx_audit_who ON audit(who,occurred_at DESC);CREATE TABLE IF NOT EXISTS alerts(id TEXT PRIMARY KEY,title TEXT NOT NULL,severity TEXT NOT NULL,status TEXT NOT NULL,evidence_json TEXT NOT NULL,created_at REAL NOT NULL,resolved_at REAL);''');c.commit();c.close()
...
class AuditIn(BaseModel):who:str=Field(min_length=1,max_length=200);what:str=Field(min_length=1,max_length=300);resource:str='';result:str='';how_method:str='';how_channel:str='';ip_address:str='';user_agent:str='';device_id:str='';session_id:str='';trace_id:str='';occurred_at:float=0;details:Dict[str,Any]=Field(default_factory=dict)
...
@app.get('/health')
def health():return {'status':'healthy','service':APP_NAME,'version':'1.1.1','timestamp':now(),'retention_days':RETENTION_DAYS,'delivery_queue':len(delivery_store.list_queue(limit=1000))}
@app.post('/events')
...
def enqueue_delivery(p:DeliveryIn,x_api_key:str=Header(default='')):
 ...
 for d in DeliveryPlanner.fanout(p.source,p.targets,p.event_type,scrub(p.payload),priority=p.priority,max_attempts=p.max_attempts):
  d.idempotency_key=(f'{p.idempotency_key}:{d.targets[0]}' if p.idempotency_key and len(p.targets)>1 else p.idempotency_key);(created if delivery_planner.enqueue(d) else duplicates).append(d.message_id)
...
def dashboard(window_seconds:int=Query(3600,ge=60,le=604800),x_api_key:str=Header(default='')):
 auth(x_api_key);since=now()-window_seconds;c=conn();rows=[dict(r) for r in c.execute('SELECT category,severity,source FROM events WHERE created_at>=?',(since,))];open_alerts=c.execute("SELECT COUNT(*) n FROM alerts WHERE status='open'").fetchone()['n'];c.close();return {'service':APP_NAME,'events_in_window':len(rows),'open_alerts':open_alerts,'by_category':dict(Counter(r['category'] for r in rows)),'by_source':dict(Counter(r['source'] for r in rows).most_common(20)),'queued':len(delivery_store.list_queue(limit=1000)),'dead_letters':len(delivery_store.list_dlq(limit=1000)),'timestamp':now()}
